...
public class CustomerController {
	...
	public ResponseEntity<String> callServiceAndGetData_Fallback() {
		...
		return ResponseEntity.ok().body(
				"CIRCUIT BREAKER ENABLED!!! No Response From Product Service at this moment. Product Service is down!!! "
	...
	/*
	 * @Autowired private ProductConsumer consumer;
	 * 
	 * @Autowired private CustomerService cservice;
	 * 
	 * @Autowired private CustomerRepository crepo;
	 * 
	 * @GetMapping(value = "/get/{id}") public ResponseEntity<Customer>
	 * getProduct(@PathVariable("id") Long pId) { return new
	 * ResponseEntity<>(cservice.getCustomer(pId), HttpStatus.OK); }
	 * 
	 * @PostMapping public ResponseEntity<List<Customer>>
	 * addNewCustomer(@RequestBody Customer customer) { Product p =
	 * customer.getProduct(); customer.setProduct(p); p.setCustomer(customer); //
	 * consumer.addNewProduct(p); //// Calling Parent return new
	 * ResponseEntity<>(cservice.saveCustomer(customer), HttpStatus.CREATED); }
	 * 
	 * @PutMapping("/update/{id}") public ResponseEntity<Customer>
	 * updateCustomer(@PathVariable("id") Long pId, @RequestBody Customer customer)
	 * { Customer c = crepo.findById(pId).get(); // Fetching Product ID to update in
	 * product microservice Product pro = c.getProduct();
	 * 
	 * c.setCustName(customer.getCustName());
	 * 
	 * pro.setProductName(customer.getProduct().getProductName());
	 * pro.setPrice(customer.getProduct().getPrice());
	 * 
	 * c.setProduct(pro); pro.setCustomer(c);
	 * 
	 * // Product uProduct = consumer.updateProduct(pro.getProductId(), pro); // //
	 * Calling Parent // uProduct.setCustomer(c); Customer updatedCustomer =
	 * cservice.updateCustomer(pId, c); return
	 * ResponseEntity.ok().body(updatedCustomer); }
	 * 
	 * @DeleteMapping("/delete/{id}") public ResponseEntity<String>
	 * deleteCustomer(@PathVariable("id") Long pId) { // Product p =
	 * cservice.getCustomer(pId).getProduct(); cservice.deleteCustomer(pId); //
	 * consumer.deleteProduct(p.getProductId()); //Calling Parent return
	 * ResponseEntity.ok().body("Customer Deleted."); }
	 * 
	 *//**
		 * Hystrix fallback method - callServiceAndGetData_Fallback
		 * 
		 * @return
		 *//*
			 * // @HystrixCommand(fallbackMethod = "callServiceAndGetData_Fallback")
			 * // @GetMapping // public ResponseEntity<List<Product>> getProducts() { //
			 * System.out.println(consumer.getClass().getName()); // List<Product> p =
			 * consumer.getProducts(); // return ResponseEntity.ok().body(p); // }
			 * 
			 * @HystrixCommand
			 * 
			 * @GetMapping(value = "/get1/{id}") public ResponseEntity<Product>
			 * getProduct1(@PathVariable("id") Long pId) { return new
			 * ResponseEntity<>(consumer.getProduct(pId), HttpStatus.OK); }
			 * 
			 * @PostMapping("/add") public ResponseEntity<String> addNewProduct(@RequestBody
			 * Product product) { consumer.addNewProduct(product); return
			 * ResponseEntity.ok().body("Product Added"); }
			 * 
			 * @PutMapping("/update1/{id}") public ResponseEntity<Product>
			 * updateProduct(@PathVariable("id") Long pId, @RequestBody Product product) {
			 * Product uProduct = consumer.updateProduct(pId, product); // Calling Parent
			 * return ResponseEntity.ok().body(uProduct); }
			 * 
			 * @DeleteMapping("/delete1/{id}") public ResponseEntity<String>
			 * deleteProduct(@PathVariable("id") Long pId) { consumer.deleteProduct(pId); //
			 * Calling Parent return ResponseEntity.ok().body("Product Deleted."); }
			 * 
			 * // public ResponseEntity<String> callServiceAndGetData_Fallback() { //
			 * System.out.println("Product Service is down!!! fallback route enabled...");
			 * // return ResponseEntity.ok().body( //
